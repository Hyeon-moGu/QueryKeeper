...
public class LazyLoadExceptionCatcher {
    ...
    public void afterThrowingLazy(Exception ex) {
        ...
        if (!m.matches()) {
            log.error("\n[QuerySentinel] LazyLoadException ❌ Unmatched message format: {}", message);
            return;
        }
        ...
        try {
            ...
            if (targetType != null) {
                ...
                QuerySentinelContext.getCurrent().markLazyLoadException(entityName);

                log.warn("\n[QuerySentinel] LazyLoadException ⚠️ DETECTED - Entity: {}, Field: {}", className,
                        fieldName);
            }
        } catch (Exception e) {
            log.error("\n[QuerySentinel] LazyLoadException ❌ FAILED to extract entity from: {}", message);
        }
    }
