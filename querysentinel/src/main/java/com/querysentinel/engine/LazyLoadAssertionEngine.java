...
public class LazyLoadAssertionEngine {
    ...
    public static void validate(String entity, int maxCount, boolean includeException, QuerySentinelContext ctx) {
        ...
        if (actual > maxCount || (includeException && hadLazyException)) {
            throw new AssertionError(String.format(
                    "\n[QuerySentinel] ExpectLazyLoad ❌ FAILED\nEntity: %s\nExpected Max: %d\nActual: %d\nException: %s",
                    entity, maxCount, actual, hadLazyException));
        }
        log.info("\n[QuerySentinel] ExpectLazyLoad ✅ PASSED - {} lazy-loaded {} time(s)", entity, actual);
    }

    public static void validateAll(int maxCount, boolean includeException, QuerySentinelContext ctx) {
        Set<String> entities = ctx.getAllEntitiesWithLazyLoad();

        if (entities.isEmpty()) {
            log.info("\n[QuerySentinel] ExpectLazyLoad ✅ PASSED - No lazy loads or exceptions detected");
            return;
        }
        ...
        for (String entity : entities) {
            ...
            if (actual > maxCount || (includeException && hadException)) {
                throw new AssertionError(String.format(
                        "\n[QuerySentinel] ExpectLazyLoad ❌ FAILED - Entity: %s, Expected Max: %d, Actual: %d, Exception: %s",
